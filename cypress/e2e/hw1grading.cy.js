...
describe("Home Page Tests", () => {
  it("Check if questions are displayed in descending order of dates.", () => {
    const qTitles = [
      ...
      "iOS development concurrent modification exception",
...
describe("New Answer Page 2", () => {
  it("Checks if a6 and a7 exist in q3 answers page", () => {
    const answers = [
      "Use locks for resources so that they can synchronize while multithreading. ",
      "You cannot have multiple threads modifying the same resource at the same time. ",
    ];
    cy.visit("http://localhost:8000");
...
describe("New Answer Page 3", () => {
  it("Checks if a8 exist in q4 answers page", () => {
    ...
    cy.contains(
      "Flutter would be an excellent tool to start with cross platform development."
    );
  });
});
